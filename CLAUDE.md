...
| app.js `hoistStandaloneButtonsOutOfCards` (2026-06-01, ensureEventVideoScript 직후) | `[[대버튼]]`/`[[중버튼]]`/`[[소버튼]]` (대괄호 2쌍 = 섹션 카드 외부 독립 배치) 의 배치를 결정론적으로 강제. 기존엔 Gemini 프롬프트 룰(L679)에만 의존 → 마커가 섹션 본문 안에 적혀 있으면 Gemini 가 문서 흐름대로 버튼을 섹션 카드 "안"에 박는 회귀 발생. 원고(`data`)에서 `[[...]]` 마커 라벨 추출 → 같은 라벨 버튼이 섹션 카드 안에 있으면 그 카드 직후 형제로 hoist. 영상/표 safety-net 과 동일한 소스 기반 결정론 패턴. idempotent (이미 밖이면 no-op), 단일 대괄호 `[버튼]` 미간섭, 동일 라벨이 `[.]`+`[[.]]` 양쪽이면 모호로 skip |
...
| js/export.js `neutralizeTaintingResourcesForCapture` (2026-06-02, prepareSheetForCapture 클론 정리 단계 — Slicer/JPG 공통) | htmlToImage 가 cross-origin `<video>` 프레임 / CSS `background-image` / SVG `<image>` / `<iframe>` 를 캔버스에 그리면 canvas 가 tainted → `toDataURL` 이 `SecurityError: Tainted canvases may not be exported` 로 실패 (슬라이스/JPG 전체 렌더 실패 회귀). `<img>` 는 `convertImagesToBase64`+`sanitizeBrokenImages` 로 이미 처리되므로 남은 오염원인 비-img 요소를 무력화: video→안전 포스터(data/blob) img 치환 or cross-origin/상대 src 면 검정 박스(data/blob src+포스터無는 유지), 인라인 cross-origin background-image 제거(색은 보존), SVG `<image>` 외부 href·iframe 제거. `[capture-diag] neutralized tainting resources:` 로 실제 오염원 카운트 로깅. |
